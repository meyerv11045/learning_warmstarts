...
def generate_training_problems(rng_seed, n):
    ...
    while i < n:
        try:
            ...
            (x0, obst, z, lam_g, stats) = gen_and_solve_problem(
                problem, np_dynamics, prev_obsts, x, u
            )
...
def gen_and_solve_problem(problem, np_dynamics, prev_obsts=None, x=None, u=None):
    ...
    if x and u:
        ...
        u_warmstart[: len(u) - 2 * skip_n_steps] = u[
            2 * skip_n_steps :
        ]  # zero controls for the nub
        cur_x = x[-4:]
        cur_u = u[-2:]
        ...
        problem.set_initial(
            problem.x[[i for i in range(len(x) + len(u))]],
            np.concatenate([x_warmstart, u_warmstart]),
        )

        obsts = generate_np_obstacles(
    ...
    problem.set_value(
        problem.p[[i for i in range(4 + 3 * problem_specs["num_obstacles"])]],
        np.concatenate([x0, obsts]),
    )
...
def generate_benchmark_problems(rng_seed, n):
    ...
    results = np.zeros(
        (
            ...
            + 3 * problem_specs["num_obstacles"]
            + 12 * n
            + 2 * problem_specs["num_constraints"],
        ),
        dtype=np.float32,
    )
    ...
    while i < n:
        try:
            (x0, obstacles, z, prev_lam_g, _) = gen_and_solve_problem(
                problem, np_dynamics
            )
            ...
            prev_x_warmstart = np.concatenate(
                [prev_x[4:], np_dynamics(prev_x[-4:], prev_u[-2:], t)]
            )
            ...
            (z_truth, lam_g_truth, stats) = solve_for_ground_truth(
                problem, x0, obstacles, prev_x_warmstart, prev_u_warmstart
            )
            ...
            print(
                f"Problem {i+1} solved in {stats[0]} iters in {stats[1]} seconds with warmstart"
            )
            i += 1
