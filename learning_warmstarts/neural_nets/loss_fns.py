...
class LagrangianFn(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        jacobian_wrt_predictions = torch.from_numpy(
            np.array(ctx.grad_fn(ctx.z, ctx.p, ctx.lamg, 0)[:240])
        )
...
class SimpleLagrangianLoss(nn.Module):
    ...
    def gen_lagrangian(self):
        ...
        l = casadi.Function(
            "L", [x, p, lamg], [f(x) - casadi.dot(lamg, (g(x, p) - b(p)))]
        )
    ...
    def forward(self, predictions, targets, params):
        ...
        """
        lamg = targets[240:]
        return LagrangianFn.apply(
            predictions, lamg, params, self.lagrangian_fn, self.lagrangian_jacobian
        )
...
class LagrangianLoss(nn.Module):
    ...
    def forward(self, predictions, targets, params):
        ...
        """
        lamg = targets[240:]
        diff = LagrangianFn.apply(
            predictions, lamg, params, self.lagrangian_fn, self.lagrangian_jacobian
        ) - LagrangianFn.apply(
            targets[:240], lamg, params, self.lagrangian_fn, self.lagrangian_jacobian
        )
        return diff**2
