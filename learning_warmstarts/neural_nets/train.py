...
from learning_warmstarts.dataset.obst_avoidance_dataset import (
    ObstacleAvoidanceBatchDataset,
    ObstacleAvoidanceDataset,
)
from learning_warmstarts.neural_nets.loss_fns import (
    ConstraintLoss,
    IpoptLoss,
    LagrangianLoss,
    SimpleLagrangianLoss,
    WeightedMSE,
)
...
class ModelTrainer:
    ...
    def load_data(self, train_folder, test_folder, load_target_duals):
        ...
        self.train_loader = torch.utils.data.DataLoader(
            dataset=train_data, batch_size=None, batch_sampler=None
        )
        self.test_loader = torch.utils.data.DataLoader(
            dataset=test_data, batch_size=None, batch_sampler=None
        )
        ...
        for epoch in range(self.epochs):
            ...
            for x, y in self.train_loader:
                ...
                moving_avg_train_loss = (
                    loss.item() + train_iters * moving_avg_train_loss
                ) / (train_iters + 1)
                train_iters += 1
            ...
            for x, y in self.test_loader:
                ...
                moving_avg_test_loss = (loss + test_iters * moving_avg_test_loss) / (
                    test_iters + 1
                )
            ...
            print(
                f"Epoch {epoch} / {self.epochs}: \t Train Loss: {train_loss:.4e} \t Test Loss {test_loss:.4e}"
            )
