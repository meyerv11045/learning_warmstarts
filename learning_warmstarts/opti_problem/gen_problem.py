...
def generate_problem(N,t,lane_width,num_obstacles,car_width,dynamics):
    ...
    for i in range(0, obstacles.shape[0], 3):
        for j in range(0, X.shape[0], 4):
            opti.subject_to((X[j] - obstacles[i])**2 + (X[j+1] - obstacles[i+1])**2 > (obstacles[i+2] + car_width)**2)
    
    # Lane Boundary Constraints
    opti.subject_to(opti.bounded(-(lane_width / 2) * MX.ones(N,1), y_poses + car_width/2, (lane_width / 2) * MX.ones(N,1)))
    opti.subject_to(opti.bounded(-(lane_width / 2) * MX.ones(N,1), y_poses - car_width/2, (lane_width / 2) * MX.ones(N,1)))
...
def generate_problem_v2():
    ...
    # acceleration limits
    problem.subject_to(problem.bounded(-500 * MX.ones(2*n,1), U, 500 * MX.ones(2*n,1)))
    ...
    velocities = select_v @ X
    problem.subject_to(problem.bounded(-200 * MX.ones(n,1), velocities, 200 * MX.ones(n,1)))
    ...
    for i in range(0, obstacles.shape[0], 3):
        for j in range(0, X.shape[0], 4):
            problem.subject_to((X[j] - obstacles[i])**2 + (X[j+1] - obstacles[i+1])**2 > (obstacles[i+2] + car_width)**2)
    
    # Lane Boundary Constraints
    problem.subject_to(problem.bounded(-(lane_width / 2) * MX.ones(n,1), y_poses + car_width/2, (lane_width / 2) * MX.ones(n,1)))
    problem.subject_to(problem.bounded(-(lane_width / 2) * MX.ones(n,1), y_poses - car_width/2, (lane_width / 2) * MX.ones(n,1)))
