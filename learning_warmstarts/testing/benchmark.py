...
class Benchmark(Test):    
    def __init__(self, hidden_layers, model_path, benchmark_folder, predict_duals, num_samples, activation_fn, version, ws_params):
        ...
        self.wandb_run = wandb.init(project='learning_warmstarts', entity='laine-lab', config=config, job_type='benchmark_predictions')

        model_at = self.wandb_run.use_artifact(version, type='model')
        model_dir = model_at.download()

        self.model = Benchmark.load_model(model_dir + '/' + model_path, self.shape, activation_fn)
    ...
    def run(self):
        ...
        dataiter = self.load_data(dataset_type='benchmark', bench_folder=self.benchmark_folder)
        ...
        for sample in dataiter: 
            ...
            self.problem.set_value(self.obstacle_param[[i for i in range(3*self.N_OBST)]], sample['obstacles'])

            iters, time = self.no_warmstart(False, sample['gt_x'], sample['gt_u'], sample['gt_lamg'])
            running_iters[0] += iters
            running_time[0] += time
            
            print(f'Sample {m:<3}, Def params: no warmstart solved in {iters} iterations ({time:.4f} s)')

            if self.ws_params:
                iters, time = self.no_warmstart(True, sample['gt_x'], sample['gt_u'], sample['gt_lamg'])
                running_iters[1] += iters
                running_time[1] += time

                print(f'Sample {m:<3}, WS Params: no warmstart solved in {iters} iterations ({time:.4f} s)')

            iters, time = self.prev_iter_warmstart(sample['prev_x'], sample['prev_u'], False, sample['gt_x'], sample['gt_u'], sample['gt_lamg'])
            running_iters[2] += iters
            running_time[2] += time

            print(f'Sample {m:<3}, Def params: prev iter solved in {iters} iterations ({time:.4f} s)')

            if self.ws_params:
                iters, time = self.prev_iter_warmstart(sample['prev_x'], sample['prev_u'], True, sample['gt_x'], sample['gt_u'], sample['gt_lamg'])
                running_iters[3] += iters
                running_time[3] += time

                print(f'Sample {m:<3}, WS params: prev iter solved in {iters} iterations ({time:.4f} s)')

            iters, time = self.primary_learned_warmstart(sample['x0'], sample['obstacles'], False, sample['gt_x'], sample['gt_u'], sample['gt_lamg'])
            running_iters[4] += iters
            running_time[4] += time

            print(f'Sample {m:<3}, Def params: primal vars solved in {iters} iterations ({time:.4f} s)')


            if self.ws_params:
                iters, time = self.primary_learned_warmstart(sample['x0'], sample['obstacles'], True, sample['gt_x'], sample['gt_u'], sample['gt_lamg'])
                running_iters[5] += iters
                running_time[5] += time

                print(f'Sample {m:<3}, WS params: primal vars solved in {iters} iterations ({time:.4f} s)')

            if self.predict_duals:
                iters, time = self.primary_dual_learned_warmstart(sample['x0'], sample['obstacles'], False, sample['gt_x'], sample['gt_u'], sample['gt_lamg'])
                running_iters[4] += iters
                running_time[4] += time

                print(f'Sample {m:<3}, Def params: primals & duals solved in {iters} iterations ({time:.4f} s)')

                if self.ws_params:
                    iters, time = self.primary_dual_learned_warmstart(sample['x0'], sample['obstacles'], True, sample['gt_x'], sample['gt_u'], sample['gt_lamg'])
                    running_iters[5] += iters
                    running_time[5] += time

                    print(f'Sample {m:<3}, WS params: primals & dua;s solved in {iters} iterations ({time:.4f} s)')
    ...
    def no_warmstart(self, use_warmstart_params, ground_truth_x, ground_truth_u, ground_truth_lam_g):
    ...
    def prev_iter_warmstart(self, x_warmstart, u_warmstart, use_warmstart_params, ground_truth_x, ground_truth_u, ground_truth_lam_g):
    ...
    def primary_learned_warmstart(self, x0, obstacles, use_warmstart_params, ground_truth_x, ground_truth_u, ground_truth_lam_g):
    ...
    def primary_dual_learned_warmstart(self, x0, obstacles, use_warmstart_params, ground_truth_x, ground_truth_u, ground_truth_lam_g):
    ...
    def solve(self):
        ...
        try:
            ...
            print(e)
            return (self.problem.debug.stats()['iter_count'], self.problem.debug.stats()['t_wall_total'])
    ...
    def create_results_dict(self, avg_iters, avg_time):
        ...
        warmstart = ['None', 'None', 'Prev Iter', 'Prev Iter', 'Neural Net', 'Neural Net']
