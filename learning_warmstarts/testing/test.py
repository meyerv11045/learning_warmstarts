...
from learning_warmstarts.opti_problem.gen_dynamics import (
    generate_list_dynamics,
    generate_MX_dynamics,
)
...
class Test:
    ...
    def __init__(self, hidden_layers, predict_duals):
        ...
        (
            ...
            self.N, self.T, self.LANE_WIDTH, self.N_OBST, self.CAR_WIDTH, self.dynamics
        )
    ...
    def load_data(
        self, dataset_type, bench_folder=None, train_folder=None, test_folder=None
    ):
        if dataset_type == "benchmark" and bench_folder:
            ...
            train_loader = torch.utils.data.DataLoader(
                train_data, batch_size=None, batch_sampler=None
            )
            test_loader = torch.utils.data.DataLoader(
                test_data, batch_size=None, batch_sampler=None
            )
    ...
    def viz_warmstart_vs_solution(
        self, x0, obstacles, warmstart_x, solution_x, filename, iters, time
    ):
        self.viz.save_compared_trajectories(
            x0, obstacles, warmstart_x, solution_x, filename, iters, time
        )
