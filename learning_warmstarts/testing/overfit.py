...
class OverfitPredictions(Test):
    def __init__(self, hidden_layers, model_path, train_folder, test_folder, use_warmstart_params, n_samples, save_trajectories, dynamic_smoothing, activation_fn, version):
        ...
        config = dict (
            warmstart_params = use_warmstart_params,
            n_samples = n_samples
        )
        if self.dynamic_smoothing:
            self.wandb_run = wandb.init(project='learning_warmstarts', entity='laine-lab', config=config, job_type='dynamic_smoothing_predictions')
        else:
            self.wandb_run = wandb.init(project='learning_warmstarts', entity='laine-lab', config=config, job_type='overfit_predictions') 
 
        model_at = self.wandb_run.use_artifact(version, type='model')
        model_dir = model_at.download()

        self.model = OverfitPredictions.load_model(model_dir + '/' + model_path, self.shape, activation_fn)

        self.train_folder = train_folder
        self.test_folder = test_folder

    def run(self):
        train_iter, test_iter = self.load_data(dataset_type='obstacle_avoidance', train_folder = self.train_folder, test_folder = self.test_folder)
        ...
        wandb.log({'train set avg iterations': avg_iters, 'train set avg time': avg_time})
    ...
    def loop_dataset(self, dataiter, table,train=True):
        ...
        for x, y in dataiter:
            ...
            try:
                ...
                self.problem.set_value(self.obstacle_param[[i for i in range(3*self.N_OBST)]], obstacles)                
                ...
                if self.dynamic_smoothing:
                    ...
                    for i in range(0, len(u_warmstart), 2):
                        cur_x = self.list_dynamics(cur_x,u_warmstart[i:i+2], self.T)
                ...
                if self.save_trajectories and m % save_img_freq == 0:
                    ...
                    self.viz_warmstart_vs_solution(x0, obstacles, x_warmstart, solved_x, filename, iters, time)
